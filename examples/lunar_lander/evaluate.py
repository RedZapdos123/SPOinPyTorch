#!/usr/bin/env python3
"""
Evaluation script for trained SPO agent on LunarLanderContinuous-v3.

This script demonstrates how to evaluate a trained SPO agent and create
...
def evaluate_agent(agent, env_name, num_episodes=100, render=False, device='cuda', create_plots=False):
    ...
    print(f"Success rate (reward >= 200): {success_rate:.2%}")
...
def create_evaluation_plots(episode_rewards, episode_lengths):
    ...
    ax5.axhline(y=200, color='red', linestyle='--', alpha=0.8, label='Success Threshold')
    ...
    success_episodes = [i for i, reward in enumerate(episode_rewards) if reward >= 200]
...
def main():
    ...
    agent = load_trained_agent(model_path, config.get_dict())
    ...
    # Optional: render a few episodes for visual evaluation
