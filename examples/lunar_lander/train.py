#!/usr/bin/env python3
"""
Training script for SPO agent on LunarLanderContinuous-v3.

This script demonstrates how to use the SPOinPyTorch library to train
a Simple Policy Optimization agent on the LunarLander environment.

Usage:
    python train.py

The script will save checkpoints and logs during training.
"""
...
def save_checkpoint(agent, epoch, reward, filepath):
...
def evaluate_agent(agent, env_name, num_episodes=10, device='cuda'):
    ...
    for _ in range(num_episodes):
        ...
        while not done:
            ...
            action_item = action.cpu().numpy()[0] if action.ndim > 1 else action.cpu().numpy()
...
def main():
    ...
    try:
        ...
        while global_step < config.total_timesteps:
            ...
            batch_actions = torch.zeros((num_steps_per_batch, num_envs, action_dim), device=device)
            ...
            b_actions = batch_actions.reshape((-1, action_dim))
            ...
            if update % config.eval_interval == 0:
                ...
                if avg_reward > best_avg_reward:
                    ...
                    save_checkpoint(agent, update, avg_reward, 'checkpoints/best_model.pth')
                    no_improvement_count = 0
            ...
            if update % config.save_interval == 0:
                save_checkpoint(agent, update, best_avg_reward, f'checkpoints/checkpoint_update_{update}.pth')
        
    except (KeyboardInterrupt, RuntimeError) as e:
        print(f"\nTraining interrupted: {e}")
        save_checkpoint(agent, update, best_avg_reward, 'checkpoints/interrupted_model.pth')
        ...
        save_checkpoint(agent, update, best_avg_reward, 'checkpoints/final_model.pth')
