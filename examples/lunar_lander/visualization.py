#!/usr/bin/env python3
"""
Training visualization script for SPO agent.

This script creates visualizations of training progress from log files
or training history JSON files.

Usage:
    python visualization.py

The script will look for training logs and create plots showing:
...
class SPOTrainingVisualizer:
    """Visualizer for SPO training progress."""
    ...
    def load_training_data(self):
        """Load training data from JSON file or parse log file."""
        try:
    ...
    def plot_training_progress(self, save_plots=True, show_plots=True):
        ...
        if 'eval_rewards' in self.data and self.data['eval_rewards']:
            ...
            ax2.axhline(y=200, color='red', linestyle='--', alpha=0.8, label='Success Threshold')
    ...
    def plot_reward_analysis(self, save_plots=True, show_plots=True):
        """Create detailed reward analysis plots."""
        ...
        ax1.axhline(y=200, color='red', linestyle='--', alpha=0.8, label='Success Threshold')
        ...
        ax2.axvline(x=200, color='orange', linestyle='--', label='Success Threshold')
        ...
        if len(eval_rewards) > 5:
            ...
            ax3.axhline(y=200, color='red', linestyle='--', alpha=0.8, label='Success Threshold')
        ...
        ax4 = axes[1, 1]
        success_rate = sum(1 for r in eval_rewards if r >= 200) / len(eval_rewards)
        ...
        values = [success_rate, final_performance/300, max(eval_rewards)/300, np.mean(eval_rewards)/300]
